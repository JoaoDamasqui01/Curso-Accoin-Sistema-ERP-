...
function mostrarMensagem(texto, tipoMensagem) { 
  ...
  mensagem.className = `mensagem ${tipoMensagem}`; 
}
...
async function carregarClientes(novoTermo = termoAtual) {
  ...
  if (pagina > totalPaginas && total) { 
    pagina = totalPaginas; 
    return carregarClientes(termoAtual); 
  }
...
function prepararEdicao(cliente) {
  id.value = cliente.CLIENTEID; 
  tipo.value = cliente.TIPO_CLIENTE || ''; 
  cpfCnpj.value = cliente.CPF_CNPJ_CLIENTE || ''; 
  nome.value = cliente.NOME_CLIENTE || '';
  if (telefone) telefone.value = cliente.TELEFONE || '';
  if (email) email.value = cliente.EMAIL || '';
  if (endereco) endereco.value = cliente.ENDERECO || '';
...
function camposValidos() {
  ...
  (!tipo.value ? tipo : !nome.value.trim() ? nome : cpfCnpj).focus(); 
  return false;
}
...
async function salvarCliente() {
  ...
  const { error } = await connSubaBase.from('CLIENTE').insert(dadosCliente);
...
async function atualizarCliente() {
  ...
  const { error } = await connSubaBase.from('CLIENTE').update(dadosAtualizados).eq('CLIENTEID', id.value);
...
async function excluirCliente(cliente) {
  ...
  if (id.value === String(cliente.CLIENTEID)) cancelarEdicao(); 
...
form.addEventListener('submit', async (evento) => { 
  ...
  if (id.value) await atualizarCliente(); 
  else await salvarCliente(); 
});

cancelar.addEventListener('click', cancelarEdicao);

document.getElementById('btnListarClientesModal').addEventListener('click', () => { 
  modal.style.display = 'flex'; 
  pagina = 1; 
  carregarClientes(busca.value); 
  busca.focus(); 
});
